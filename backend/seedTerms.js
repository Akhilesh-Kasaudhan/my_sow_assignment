...
const englishTerms = `BY clicking Invoice Now, you choose to register according to the information that you have typed in and the text on the registration page and the terms here, and you at the same time accept the terms here.
...
Have a great day!`;
const swedishTerms = `GENOM ATT klicka på Fakturera Nu så väljer ni att registrera enligt den information som ni har lagt in och texten på registrerings sidan och villkoren här, och accepterar samtidigt villkoren här.
...
async function insertTerms() {
  try {
    ...
    await db.Terms.bulkCreate([
      { language: "en", content: englishTerms },
      { language: "sv", content: swedishTerms },
    ]);

    console.log("Terms inserted successfully!");
    process.exit();
  } catch (err) {
    console.error("Error inserting terms:", err);
    process.exit(1);
  }
}
